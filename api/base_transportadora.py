...
class BaseTransportadora(ABC):
    ...
    def buscar_pedido_especifico(self, cpf: str, numero_fiscal: str) -> Optional[Dict]:
        ...
        try:
            ...
            if not dados:
                logger.warning(f"{self.nome}: Nenhum dado retornado para o CPF")
                return None

            pedidos = self.extrair_pedidos(dados)
            if not pedidos:
                logger.warning(f"{self.nome}: Nenhum pedido encontrado")
                return None
            ...
            logger.warning(f"{self.nome}: NF {numero_fiscal} não encontrada nos pedidos do CPF")
            return None
